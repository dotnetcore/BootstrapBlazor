...
public partial class CardUpload<TValue>
{
    ...
    private bool IsImage(UploadFile item)
    {
        ...
        {
            ret = IsBase64Format(item) || CheckExtensions(item.FileName ?? item.PrevUrl ?? "");
        }
        return ret;

    }
    private static bool IsBase64Format(UploadFile item) => !string.IsNullOrEmpty(item.PrevUrl) && item.PrevUrl.StartsWith("data:image/", StringComparison.OrdinalIgnoreCase);

    private static bool CheckExtensions(string fileName)
    {
        var extension = Path.GetExtension(fileName).ToLowerInvariant();
        var exist = _mappings.Where(p => p.Key == extension&& p.Value.StartsWith("image/")).Count();
        if (exist >= 0)
        {
            return true;
        }
