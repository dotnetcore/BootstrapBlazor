...
export class ImageViewer extends BlazorComponent {
    _init() {
        ...
        this._preview = this._element.classList.contains('is-preview')
        if (this._preview) {
            this._body = document.querySelector('body')
            this._wrapper = this._element.querySelector('.bb-viewer-wrapper')
            this._prevImg = this._wrapper.querySelector('.bb-viewer-canvas > img')
            this._closeButton = this._element.querySelector('.bb-viewer-close')
            this._prevButton = this._element.querySelector('.bb-viewer-prev')
            this._nextButton = this._element.querySelector('.bb-viewer-next')
            this._zoomOut = this._element.querySelector('.fa-magnifying-glass-plus')
            this._zoomIn = this._element.querySelector('.fa-magnifying-glass-minus')
            this._rotateLeft = this._element.querySelector('.fa-rotate-left')
            this._rotateRight = this._element.querySelector('.fa-rotate-right')
    ...
    _setListeners() {
        ...
        // 上一张按钮处理事件
        EventHandler.on(this._prevButton, 'click', () => {
            this._index--
            if (this._index < 0) {
        ...
        // 下一张按钮处理事件
        EventHandler.on(this._nextButton, 'click', () => {
            this._index++
    ...
    _updateImage(index) {
        ...
        const img = this._wrapper.querySelector('.bb-viewer-canvas > img')
    ...
    _execute(args) {
        ...
        // 移动到文档结尾
        this._body.appendChild(this._wrapper)
    ...
    _dispose() {
        if (this._preview) {
            ...
            // 上一张按钮处理事件
            EventHandler.off(this._prevButton, 'click')

            // 下一张按钮处理事件
            EventHandler.off(this._nextButton, 'click')
