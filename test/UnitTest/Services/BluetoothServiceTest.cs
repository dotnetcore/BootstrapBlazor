...
public class BluetoothServiceTest : BootstrapBlazorTestBase
{
    ...
    public async Task RequestDevice_Ok()
    {
        ...
        var device = await bluetoothService.RequestDevice(services.GetServicesList(), CancellationToken.None);
        ...
        var val = await device.ReadValue("battery_service", "battery_level", CancellationToken.None);
        Assert.Equal([0x31], val);

        var v = await device.GetBatteryValue(CancellationToken.None);
        Assert.NotNull(v);
        ...
        await device.Disconnect(CancellationToken.None);
    ...
    public async Task GetPrimaryServices_Ok()
    {
        ...
        var bluetoothService = Context.Services.GetRequiredService<IBluetooth>();
        var device = await bluetoothService.RequestDevice(token: CancellationToken.None);
        Assert.NotNull(device);

        await device.Connect(CancellationToken.None);
        var v = await device.GetPrimaryServices(CancellationToken.None);
    ...
    public async Task GetCharacteristics_Ok()
    {
        ...
        await device.Connect(CancellationToken.None);
        var service = await device.GetPrimaryService("battery_service", CancellationToken.None);
        Assert.NotNull(service);

        var v = await service.GetCharacteristics(CancellationToken.None);
    ...
    public async Task ReadValue_null()
    {
        ...
        await device.Connect(CancellationToken.None);
        var v = await device.ReadValue("battery_service", "battery_level", CancellationToken.None);
        Assert.Null(v);

        var data = await device.GetBatteryValue(CancellationToken.None);
        Assert.Null(v);
        ...
        var characteristic = await service.GetCharacteristic("battery_level", CancellationToken.None);
        ...
        v = await characteristic.ReadValue(CancellationToken.None);
        Assert.Null(v);
    }
    ...
    public async Task GetAvailability_Ok()
    {
        ...
        await bluetooth.GetAvailability(CancellationToken.None);
    ...
    public async Task GetDeviceInfo_null()
    {
        ...
        await device.Connect(CancellationToken.None);
        var v = await device.GetDeviceInfo(CancellationToken.None);
    ...
    public async Task GetCurrentTime_null()
    {
        ...
        await device.Connect(CancellationToken.None);
        var v = await device.GetCurrentTime(CancellationToken.None);
        Assert.NotNull(v);
    ...
    public async Task Notifications_Ok()
    {
        ...
        notification = await characteristic.StartNotifications(payload => { return Task.CompletedTask; }, CancellationToken.None);
        ...
        notification = await characteristic.StopNotifications(CancellationToken.None);
        Assert.True(notification);
